...
run_multi_lstm=function(Y,indice,horizon){
...
run_selected_lstm <- function(Y, indice=1, horizon=1, selected){
  comp=princomp(scale(Y,scale=FALSE))
  Y2 = cbind(Y, comp$scores[,1:4]) %>% as.data.frame()
  Y3 = lapply(Y2, normalize) %>% as.data.frame() %>% as.matrix()
  aux=embed(Y3,4+horizon)
  ...
  # =============================================================
  # De-normalization
  pred = model %>% predict(X.arr) %>% denormalize(min(Y2[,indice]),max(Y2[,indice])) %>% as.numeric()
  return(list("model"=model,"pred"=pred))
}
...
selected.lstm.rolling.window=function(Y,nprev,indice=1,lag=1, selected){
  
  save.pred=matrix(NA,nprev,1)
  
  for(i in nprev:1){
    Y.window=Y[(1+nprev-i):(nrow(Y)-i),] %>% as.data.frame()
    lstm=run_selected_lstm(Y.window,indice,lag, selected)
    save.pred[(1+nprev-i),]=as.numeric(lstm$pred) # Note as.numeric()
    cat("iteration",(1+nprev-i),"\n")
  }
